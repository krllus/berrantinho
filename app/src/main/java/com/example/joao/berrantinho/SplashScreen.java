...
public class SplashScreen extends AppCompatActivity {
    ...
    switch (requestCode) {
      case REQUEST_AUTHENTICATION:
        if (resultCode == RESULT_OK) {
          ...
          startupIntent.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TASK
  ...
  private void checkUserLoggedIn() {
    ...
    if (!Utils.isPlayServicesAvailable(this)) {
      Toast.makeText(this, "google play services not available in device.", Toast.LENGTH_SHORT)
          .show();
      //finish();
    }
    ...
    SharedPreferences sharedPreferences = getSharedPreferences(
        getString(R.string.preference_account_file_key), Context.MODE_PRIVATE);
    String accountName =
        sharedPreferences.getString(getString(R.string.preference_account_name), null);
  ...
  /**
   * Show all the accounts registered on the account manager. Request an auth token upon user
   * select.
   */
